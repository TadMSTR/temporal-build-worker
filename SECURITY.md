...
- **GitHub private disclosure:** Use the [Security tab](https://github.com/TadMSTR/temporal-build-worker/security/advisories/new) to submit a private advisory.
- **Email:** Send a description to `security.i9v75@8alias.com` with the subject line `[temporal-build-worker] Security Report`.
...
- Workflow injection via malformed build plan inputs or activity payloads
- Task token leakage enabling unauthorized workflow completion signals
- SSRF via Temporal server or Vault endpoint configurations
- Information disclosure through workflow execution logs or activity outputs
- Dependency vulnerabilities with a plausible exploitation path in temporal-build-worker's usage
